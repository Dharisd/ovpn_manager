...
def dump_file_in_mem(material, format=crypto.FILETYPE_PEM):
    ...
    if isinstance(material, crypto.X509):
        ...
        raise Exception("Don't know how to dump content type to file: {} (%r)" % (type(material), material))
...
def load_from_file(materialfile, objtype, format=crypto.FILETYPE_PEM):
    if objtype is crypto.X509:
        ...
        raise Exception("Unsupported material type: {}" % (objtype,))
...
def make_new_ovpn_file(ca_cert, ca_key, clientname, serial, commonoptspath, filepath):
    ...
    print(clientkey)
    ovpn = f"{common}<ca>\n{cacertdump}</ca>\n<cert>\n{clientcert}</cert>\n<key>\n{clientkey}</key>\n".format(common, cacertdump, clientcert, clientkey)
...
if __name__ == "__main__":
    make_new_ovpn_file("/etc/openvpn/ca.crt", "/etc/openvpn/ca.key", "justasictdest", 0x0C, "common.txt", "lsd.ovpn")
    print("Done")